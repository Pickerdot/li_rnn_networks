...
def hard_max2(x, n):
    ...
    x = x / ld[0, -n]
    result = np.floor(x)
    return np.where(result < 1, 0, 1)


def answer_rate(rate, input, target):
    answer = hard_max(input)
    div = target - answer
    if (np.max(div) == 0):
        rate = rate + 1
    return rate
...
def answer_rate2(rate, input, target, n):
    answer = hard_max2(input, 2)
    div = target - answer
    if (np.max(div) == 0):
        rate = rate + 1
    return rate
...
class max2_network:
    ...
    def traning(self, test_data, target_data, epoch):
        ...
        for i in range(epoch):
            ...
            self.correct_num = answer_rate2(self.correct_num, out2, t, 2)
        ...
        self.bo = bo
        print("bo:", self.bo)
        self.correct_rate = self.correct_num / i

        return self.loss_memo, self.correct_rate
    ...
    def traning(self, test_data, target_data, epoch):
        ...
        for i in range(epoch):
            ...
            if i % 100 == 0:
                ...
                # lrs2 = self.Third_layer.viewlr()
            loss = self.test_loss_layer.forward(out2, t)
            self.correct_num = answer_rate(self.correct_num, out2, t)
            ...
            if i % 500 == 0 and i != 0:
                ...
                # print("out1:", out1)
                print("out2:", out2)
                print("out3:", hard_max(out2))
                print("t:", t)
                print("c_num:", self.correct_num)
                # print("lrs1:", lrs1)
                # print("lrs2:", lrs2)
                print("New_lr:", New_lr)
                print("_________")
